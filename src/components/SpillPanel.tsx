...
function regnUt(start: number, ringer: Ring[]) {
  ...
  for (const [i, r] of ringer.entries()) {
    ...
    if (tall.length) steg.push(`Ring ${i + 1} (${FARGER[r.op].navn} ${FARGER[r.op].tegn} ${tall.join(', ')}): ${Number.isInteger(v) ? v : v.toFixed(3)}`)
  }
  return { svar: v, steg }
}
...
function Dartkalkulator() {
  ...
  const res = useMemo(() => (start ? regnUt(Number(start), ringer) : null), [start, ringer])
  const oppdater = (i: number, endring: Partial<Ring>) => setRinger((r) => r.map((x, j) => (j === i ? { ...x, ...endring } : x)))
  ...
  return (
    <section className="rounded-2xl border-2 border-primary/40 bg-card p-4">
      ...
      <p className="mt-0.5 text-[13px] text-muted-foreground">Skriv inn tallet i midten, og så hver ring innenfra og ut: fargen og tallene feltene peker på.</p>

      <label className="mt-3 block text-[13.5px] font-semibold" htmlFor="dart-start">
        Tallet i midten
      </label>
      <Input id="dart-start" inputMode="numeric" className="mt-1 font-mono text-lg" placeholder="f.eks. 13" value={start} onChange={(e) => setStart(e.target.value.replace(/[^0-9]/g, ''))} />
        ...
        {ringer.map((r, i) => (
          <div key={i} className="rounded-xl bg-slate-50 p-3">
            <div className="flex items-center justify-between">
              ...
              {ringer.length > 1 && (
                <Button variant="ghost" size="icon-sm" onClick={() => setRinger((x) => x.filter((_, j) => j !== i))} aria-label={`Fjern ring ${i + 1}`}>
                  <Trash2 />
                </Button>
              )}
            </div>
            <div className="mt-2 grid grid-cols-4 gap-1.5" role="radiogroup" aria-label={`Farge på ring ${i + 1}`}>
              {(Object.keys(FARGER) as Op[]).map((op) => (
                <button
                  ...
                  className={cn('min-h-11 rounded-lg text-[13.5px] font-semibold ring-offset-2 transition', FARGER[op].klasse, r.op === op ? 'ring-2 ring-slate-900' : 'opacity-45')}
                >
      ...
      <Button variant="outline" className="mt-3 w-full" onClick={() => setRinger((r) => [...r, { op: 'add', tall: '' }])}>
        <Plus /> Legg til ring
      </Button>

      {res && (
        <div className="mt-4 rounded-xl bg-slate-900 p-4 text-white" aria-live="polite">
          <p className="text-[12px] font-semibold tracking-wider text-slate-300 uppercase">Svar</p>
          <p className="font-mono text-3xl font-bold">{Number.isInteger(res.svar) ? res.svar : res.svar.toFixed(3)}</p>
          {(!Number.isInteger(res.svar) || res.svar < 1 || res.svar > 999) && (
            <p className="mt-1 text-[13px] text-amber-300">Svaret skal være et helt tall mellom 1 og 999. Sjekk fargene og tallene.</p>
          )}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border border-amber-200 bg-amber-50 p-3.5 text-[14.5px] leading-relaxed text-amber-900">
        ...
        <p className="mt-0.5">Serveren som gir koden svarer «not_configured». Spillene laster, men ingen kan få koden før Horde skrur den på.</p>
      </div>

      <Button asChild className="w-full">
        <a href="https://horde.no/secret/kodejakten" target="_blank" rel="noopener">
      ...
      <ol className="space-y-2.5">
        {SPILL.map((s) => (
          <li key={s.nr} className="rounded-2xl border bg-card p-4">
            ...
            <p className="mt-1 text-[14.5px] leading-relaxed text-slate-700">{s.hva}</p>
            <ul className="mt-2 space-y-1.5">
              {s.tips.map((t) => (
                <li key={t} className="flex gap-2 text-[14px] leading-snug text-slate-600">
