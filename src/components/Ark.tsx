...
export type Fane = 'lag' | 'hint' | 'tavla' | 'stream'
...
export function Ark({ fane, onFane, hoyde, onHoyde, desktop, innhold, antallHint }: Props) {
  ...
  return (
    <aside
      ...
      className={cn(
        ...
        desktop ? 'top-3 bottom-3 left-3 w-[25rem] rounded-3xl border' : 'inset-x-0 bottom-0 rounded-t-3xl border-t',
      )}
      style={desktop ? undefined : { height: hoyder.full }}
      aria-label="Panel"
    >
      <Tabs
        value={fane}
      ...
      >
        ...
        >
          ...
          <TabsList className="grid h-10 w-full grid-cols-4">
            {FANER.map((f) => (
              <TabsTrigger key={f.id} value={f.id} className="h-full text-[13px]">
                {f.navn}
