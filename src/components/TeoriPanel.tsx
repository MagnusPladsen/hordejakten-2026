...
function utslagFor(t: Teori, aktive: Bevis[]) {
  const alle = aktive
    .map((b) => ({ b, f: b.faktor(t) }))
    .filter(({ f }) => Math.abs(Math.log(f)) > 0.05);
...
export function TeoriPanel({
  ...
  const rangert = [...TEORIER_LISTE].sort(
    (a, b) => prosent[b.id] - prosent[a.id],
  );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-4 @[46rem]:grid-cols-2 @[46rem]:items-center">
        <div>
          ...
          <p className="mt-1 text-sm leading-relaxed text-muted-foreground">
            Prosenten viser hvor godt hver teori passer med alle hintene samlet.
            Trykk på en teori for å se hvorfor.
          </p>
        </div>
            ...
            {(
              [
                {
                  id: "alt",
                  navn: "Alt vi har",
                  tekst: "Hint + det folk sier",
                },
                {
                  id: "hint",
                  navn: "Bare hint",
                  tekst: "Det vi har sett selv",
                },
              ] as const
            ).map((m) => (
              <button
              ...
              >
                ...
                <span
                  className={cn(
                    "block text-[12px]",
                    modus === m.id ? "text-slate-300" : "text-muted-foreground",
                  )}
                >
                  {m.tekst}
                </span>
      ...
      <ol className="grid items-start gap-2 @[46rem]:grid-cols-2">
        {rangert.map((t, i) => {
          ...
          return (
            <li
              ...
              className={cn(
                ...
                i === 0 && "border-slate-300 shadow-sm",
              )}
            >
              <button
                type="button"
                className="w-full p-3.5 text-left"
                onClick={() => setApen(erApen ? null : t.id)}
                aria-expanded={erApen}
              >
                <div className="flex items-center gap-3">
                  <span
                    className="w-14 shrink-0 font-mono text-xl font-semibold tabular-nums"
                    style={{ color: t.farge }}
                  >
                    {p < 1 ? "<1" : Math.round(p)}%
                  </span>
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[13px] text-muted-foreground">
                      {t.kort}
                      {t.kjoretid != null &&
                        ` · ${t.kjoretid.toFixed(1).replace(".", ",")} t fra Oslo`}
                    </p>
                  </div>
                  <ChevronDown
                    className={cn(
                ...
                <div className="mt-2.5 h-1.5 overflow-hidden rounded-full bg-slate-100">
                  <div
                    ...
                    style={{ width: `${Math.max(1, p)}%`, background: t.farge }}
                  />
                </div>
                {!erApen && (opp.length > 0 || ned.length > 0) && (
                  <p className="mt-2 text-[13.5px] leading-snug text-slate-600">
                    ...
                    {ned.length > 0 && (
                      <>
                        <span className="font-semibold text-red-700">Mot:</span>{" "}
                        {kortTittel(ned[0].b)}
                      </>
                    )}
                  </p>
              ...
              {erApen && (
                <div className="border-t border-dashed px-3.5 pt-3 pb-3.5">
                  {opp.length === 0 && ned.length === 0 && (
                    <p className="text-[13px] text-muted-foreground">
                      Ingen av hintene som er på, sier noe spesielt om denne
                      teorien.
                    </p>
                  )}
                  <ul className="space-y-1.5">
                    {[...opp, ...ned].map(({ b, f }) => (
                      ...
                      >
                        {f > 1 ? (
                          <ArrowUp className="mt-0.5 size-3.5 shrink-0 text-emerald-600" />
                        ) : (
                          <ArrowDown className="mt-0.5 size-3.5 shrink-0 text-red-600" />
                        )}
                        <span className="min-w-0 flex-1">{b.tittel}</span>
                        <span
                          className={cn(
                            "shrink-0 font-mono text-[12px] font-semibold",
                            f > 1 ? "text-emerald-700" : "text-red-700",
                          )}
                        >
      ...
      <p className="px-1 text-[13px] leading-relaxed text-muted-foreground">
        Prosentene er et anslag basert på skjønn, ikke fasit. Se «Hvordan regnes
        prosenten ut?» under.
      </p>
      ...
      >
          ...
          <AccordionTrigger className="py-3.5">
            <span>
              <span className="block text-[15px] font-semibold">
                Juster hvilke hint som teller
              </span>
              <span className="block text-xs font-normal text-muted-foreground">
                {aktive.length} av {iModus.length} er med. Slå av det du ikke
                tror på.
              </span>
            </span>
          </AccordionTrigger>
                  ...
                  >
                    <span className="min-w-0">
                      <span className="block text-[14.5px] leading-snug font-semibold">
                        {b.tittel}
                        {b.kilde === "folk" && (
                          <span className="ml-1.5 rounded bg-violet-50 px-1 py-px text-[11px] font-semibold text-violet-700 ring-1 ring-violet-200">
                            Folk sier
                          </span>
                        )}
                      </span>
                    ...
                    <Switch
                      checked={pa}
                      onCheckedChange={(v) => onVeksleBevis(b.id, v)}
                      aria-label={b.tittel}
                    />
                  </label>
                );
        ...
        <AccordionItem value="metode" className="border-t">
          <AccordionTrigger className="py-3.5 text-[15px] font-semibold">
            Hvordan regnes prosenten ut?
          </AccordionTrigger>
          <AccordionContent className="space-y-2 text-[14.5px] leading-relaxed text-slate-600">
            <p>
              Alle teoriene starter likt. «Et helt annet sted» starter fire
              ganger så høyt, fordi teori-sirklene er små og resten av landet er
              stort.
            </p>
            <p>
              Hvert hint gir en faktor per teori. ×2 betyr at hintet gjør
              teorien dobbelt så sannsynlig, ×0,5 halvparten så sannsynlig.
              Faktorene ganges sammen, og alt skaleres til 100 %.
            </p>
            <p>
              Faktorene er skjønn, ikke fasit. De viktigste er flyet Anja pekte
              på og været.
            </p>
          </AccordionContent>
        </AccordionItem>
  ...
  return (
    <div className="relative rounded-2xl bg-slate-900 p-4 pr-10 text-white">
      <button
        type="button"
        onClick={lukk}
        className="absolute top-1 right-1 grid size-11 place-items-center text-slate-400 hover:text-white"
        aria-label="Lukk"
      >
        <X className="size-4" />
      </button>
      ...
      <ol className="mt-2 space-y-1.5 text-[14.5px] leading-snug text-slate-200">
        <li>
          <b className="text-white">Teorier</b> viser hvor kassen mest
          sannsynlig står, i prosent.
        </li>
        <li>
          <b className="text-white">Kart</b> lar deg velge hva du ser. Trykk på
          kartet for detaljer om et sted.
        </li>
        <li>
          <b className="text-white">Hint</b> samler alle hint, koder og hva folk
          tror.
        </li>
      </ol>
      <button
        type="button"
        onClick={lukk}
        className="mt-3 min-h-11 rounded-full bg-white px-5 text-[14.5px] font-semibold text-slate-900"
      >
        Skjønner
      </button>
    </div>
  ...
  return (
    ...
    >
      <div className="p-4">
        ...
        <p className="mt-1.5 text-[14.5px] leading-relaxed text-red-50">
          {nyest.tekst}
        </p>
        {nyest.lenke && (
          <a
      ...
      {eldre.length > 0 && (
        <ul className="divide-y divide-white/20 border-t border-white/20 bg-black/10 @[46rem]:columns-2 @[46rem]:gap-0 @[46rem]:divide-y-0 [&>li]:break-inside-avoid @[46rem]:[&>li]:border-b @[46rem]:[&>li]:border-white/20">
          {eldre.map((n) => (
            <li
              key={n.tittel}
              className="px-4 py-2.5 text-[13.5px] leading-snug"
            >
              <span className="font-semibold">{n.tittel}.</span>{" "}
              <span className="text-red-100">{n.tekst}</span>
            </li>
          ))}
          {resten.length > 3 && (
            <li>
              <button
                type="button"
                onClick={() => setAlle((a) => !a)}
                className="min-h-11 w-full px-4 text-left text-[13.5px] font-semibold underline underline-offset-2"
              >
